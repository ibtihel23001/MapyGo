...
import { z } from 'zod'
import { useAccountants, useCreateUser, useToggleUserStatus, useDeleteUser } from '../../../API/hooks'
import {
...
// Must match backend: min 8 chars, at least one uppercase, at least one number
const schema = z.object({
  firstName: z.string().min(1, 'Required'),
  lastName:  z.string().min(1, 'Required'),
  username:  z.string().min(3, 'Min 3 chars').regex(/^[a-zA-Z0-9_]+$/, 'Letters, numbers and underscores only'),
...
export default function AccountantsPage() {
  ...
  function onSubmit(d: FormData) {
    createMut.mutate({ ...d, roleSlug: 'accountant' }, {
  ...
  return (
    <div>
      <PageHeader
        title="Accountants"
        subtitle="Manage your agency's accountants"
        action={<Button onClick={() => setModal(true)}><Plus size={16} /> Add Accountant</Button>}
      />
      ...
      <Modal open={modal} onClose={() => { setModal(false); reset(); createMut.reset?.() }} title="Add Accountant" size="md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          >
            ...
            {!errors.password && <p className="mt-1 text-xs text-slate-400">Min 8 chars, one uppercase letter, one number</p>}
          </FormField>
          <div className="flex justify-end gap-2">
            <Button variant="secondary" type="button" onClick={() => { setModal(false); reset(); createMut.reset?.() }}>Cancel</Button>
      ...
      <ConfirmDialog
        ...
        message="Delete this accountant account? This cannot be undone."
